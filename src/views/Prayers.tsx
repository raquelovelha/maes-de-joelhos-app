...
const Prayers: React.FC<any> = ({ prayers = [], filhos = [], onNavigate }) => {
  ...
  const categories = [
    ...
    { id: 'PROTECAO', label: 'Proteção, Livramentos e Batalha Espiritual', icon: 'fa-shield-halved', color: 'bg-blue-50', text: 'text-blue-500' },
    { id: 'MINISTERIO', label: 'Vida Espiritual e Ministério', icon: 'fa-fire-alt', color: 'bg-orange-50', text: 'text-orange-500' },
    { id: 'CARATER', label: 'Caráter, Valores e Emoções', icon: 'fa-gem', color: 'bg-purple-50', text: 'text-purple-500' },
    { id: 'SAUDE', label: 'Saúde e Necessidades Humanas', icon: 'fa-heart-pulse', color: 'bg-emerald-50', text: 'text-emerald-500' },
    { id: 'FUTURO', label: 'Educação, Futuro e Propósito', icon: 'fa-graduation-cap', color: 'bg-indigo-50', text: 'text-indigo-500' },
    { id: 'FAMILIA', label: 'Relacionamentos e Família', icon: 'fa-house-chimney-heart', color: 'bg-pink-50', text: 'text-pink-500' },
    { id: 'MISSOES', label: 'Missões e Sociedade', icon: 'fa-globe-americas', color: 'bg-cyan-50', text: 'text-cyan-500' },
    { id: 'GRATIDAO', label: 'Gratidão e Esperança', icon: 'fa-sun', color: 'bg-amber-50', text: 'text-amber-500' },
  ];
  ...
  const filteredPrayers = useMemo(() => {
    ...
    return prayers.filter((p: any) => {
      ...
      // Filtro flexível: busca pelo nome exato ou se o tema contém parte do nome principal
      return temaBanco === config.label || temaBanco.toLowerCase().includes(config.label.split(' ')[0].toLowerCase());
    });
    ...
    <div className="flex flex-col gap-6 pb-24 pt-4 px-2">
      ...
      {!selectedCategory ? (
        <div className="flex flex-col gap-3 animate-fadeIn">
          <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-2 mb-1">Temas de Clamor</h3>
          {categories.map(cat => (
            <button 
              key={cat.id} 
            ...
            >
              ...
              <div className="flex flex-col flex-1">
                ...
                <span className={`text-[9px] font-black uppercase mt-1 ${cat.text}`}>Explorar</span>
              </div>
        ...
        <div className="flex flex-col gap-4 animate-fadeIn">
          ...
          <div className="space-y-3">
            {selectedCategory === 'FILHOS' ? (
              ...
              )) : <p className="text-center py-20 text-gray-400">Nenhum pedido nesta categoria.</p>
            )}
          </div>
        </div>
      )}
