  ...
  return (
    <div className="flex flex-col gap-6 pb-24 pt-4 px-1">
      ...
      <div className="grid grid-cols-3 gap-2 px-1">
        {categories.map(cat => (
          <button key={cat.id} onClick={() => setSelectedCategory(cat.id)} className="flex flex-col items-center justify-center p-3 rounded-[2rem] bg-white shadow-sm border border-gray-50">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center text-gray-400 bg-gray-50 mb-2">
              <i className={`fa-solid ${cat.icon}`}></i>
            </div>
            <span className="text-[8px] font-black uppercase text-gray-500">{cat.label}</span>
          </button>
        ))}
      </div>

      {/* MODAL DE FILHOS - CORRIGIDO */}
      {showFilhosModal && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setShowFilhosModal(false)}></div>
          <div className="relative bg-white w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl animate-popIn">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold text-[#2D1B4D]">Pedidos dos Filhos</h2>
              <button onClick={() => setShowFilhosModal(false)} className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-500">
            ...
            <div className="space-y-4 max-h-[50vh] overflow-y-auto">
              {filhos.length > 0 ? filhos.map((f: any, i: number) => (
                <div key={i} className="bg-blue-50 p-4 rounded-2xl border border-blue-100">
                  ...
                  <p className="text-sm text-[#2D1B4D] italic">"{f.pedido || f.clamor || 'Sem pedido registrado'}"</p>
                </div>
              )) : <p className="text-center text-gray-400 text-sm">Nenhum pedido encontrado.</p>}
            </div>
          </div>
        </div>
      )}

{/* MODAL DE CATEGORIAS - VERSÃO ULTRA COMPATÍVEL */}
      {selectedCategory && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
          <div className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl overflow-hidden flex flex-col">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-white">
              <h2 className="text-xl font-bold text-[#2D1B4D]">
              ...
              <div className="space-y-4">
                {filteredPrayers.length > 0 ? filteredPrayers.map((p: any) => (
                  <div key={p.id} className="bg-gray-50 p-5 rounded-2xl border border-gray-200">
                    <p className="text-base text-[#2D1B4D] leading-relaxed mb-2 font-medium">
                      {p.description || p.texto}
                    </p>
                    {p.verse && (
                      <div className="inline-block bg-white px-3 py-1 rounded-full border border-pink-100">
                        <span className="text-[10px] font-black text-[#FF4DAD] uppercase">
                  ...
                  <div className="text-center py-10">
                    <p className="text-gray-400 text-sm">Carregando motivos...</p>
                  </div>
                )}
              </div>
            </div>
