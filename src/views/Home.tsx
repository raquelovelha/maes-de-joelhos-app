...
const Home: React.FC<HomeProps> = ({ profile }) => {
  // Dados vindos do Firebase 'usuarios'
  const pedidosRealizados = profile?.pedidosConcluidos || 0;
  const minutosHoje = profile?.minutosIntercedidos || 0; 
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-32 px-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        {/* PEDIDOS - Agora atualiza com o banco */}
        <div className="bg-white rounded-[2.5rem] p-6 border border-gray-100 shadow-sm flex flex-col items-center text-center">
          ...
          <p className="text-3xl font-black text-brand-dark">{pedidosRealizados}</p>
        </div>
        ...
        <div className="bg-white rounded-[2.5rem] p-6 border border-gray-100 shadow-sm flex flex-col items-center text-center">
          ...
          <p className="text-3xl font-black text-brand-dark">{minutosHoje}m</p>
        </div>
      </div>

      {/* LOGO GERAÇÃO COMPROMISSO - Visível e centralizada */}
