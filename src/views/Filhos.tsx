...
// Inputs: children, onAccept, onAddChild, onAddRequest, onToggleRequest, onRegisterPrayer | Outputs: UI de intercessão por filhos.
...
const FilhosView: React.FC<FilhosProps> = ({ 
  ...
  const toggleModal = (key: keyof typeof modals, val: boolean) => {
      ...
      if (!val) setWhatsappError(''); // Reset error when closing
  };
  ...
  const handleAcceptChild = () => {
    ...
    onAccept(selectedChild.id);
    setSelectedChild(prev => prev ? { ...prev, status: 'active' } : null);
    alert("Cadastro confirmado! Agora você pode adicionar pedidos de oração.");
  };

  const handleSaveRequest = () => {
    if (!selectedChild || !reqData.text.trim()) return alert("Por favor, descreva o pedido.");
    
    const newRequest: ChildPrayerRequest = {
      id: Date.now().toString(),
      request: reqData.text,
      verse: reqData.verse,
      status: 'em_oracao',
    ...
    onAddRequest(selectedChild.id, newRequest);
    setSelectedChild(prev => prev ? { ...prev, individualRequests: [newRequest, ...prev.individualRequests] } : null);
    
    setReqData({ text: '', verse: '', notes: '' });
    setIsAddingRequest(false);
  };

  const handleToggleStatus = (reqId: string) => {
    if (!selectedChild) return;
    onToggleRequest(selectedChild.id, reqId);
    
    setSelectedChild(prev => {
      if (!prev) return null;
      return {
        ...prev,
        individualRequests: prev.individualRequests.map(r => {
          if (r.id === reqId) {
            const newStatus = r.status === 'em_oracao' ? 'respondido' : 'em_oracao';
            return { ...r, status: newStatus, resolvedAt: newStatus === 'respondido' ? new Date().toISOString() : undefined };
          }
          return r;
        })
      };
    ...
    <div className="space-y-6 pb-40 animate-fadeIn"> 
      <header className="flex items-center justify-between">
        <div>
          ...
          {/* Logo redundante removida daqui conforme solicitado */}
        </div>
      ...
      {/* --- ÁREA DO RODAPÉ INSTITUCIONAL REAJUSTADA --- */}
      ...
      {/* Modal Detalhes/Pedidos omitido aqui mas deve ser mantido conforme seu código original */}
    </div>
  );
};
